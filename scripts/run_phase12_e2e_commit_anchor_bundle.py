#!/usr/bin/env python3
"""Phase 12 runner: end-to-end (episode 1~3) evaluation with **commit anchors derived from model evidence**.

What changes vs Phase 11:
- Commit anchors used in episodes 1/2 can be selected from:
  - opened_supporting (baseline / legacy)
  - model_evidence (LLM output `evidence` field)
  - hybrid (union)
- Episode 3 enforces anchor reuse: final answering is restricted to commit anchors (plus pivot update message).

This runner:
- Generates 3 pivot types (retention_flip, entity_switch, constraint_add) with per-type pivot rates.
- Runs baselines + GoC variants at a fixed large budget (default 4000) to isolate commit-anchor effects.
- Produces a self-contained experiment bundle under experiment_bundles/.

Usage:
  python scripts/run_phase12_e2e_commit_anchor_bundle.py --dotenv .env
"""
...
def _link_data_dir(src_root: Path, dst_out_dir: Path) -> None:
    """Ensure compare out_dir has a `data/` directory that contains `worlds/` and `tasks/`.

    Generator output is typically:
      <out_dir>/data/worlds/*.jsonl

    Earlier phases sometimes passed the *root* generate out_dir (which contains a `data/` subdir)
    instead of the `data/` directory itself. To be robust, we auto-detect whether `src_root`
    already points to the `data/` directory.
    """

    # Pick the correct source directory to link/copy.
    ...
    for cand in candidates:
        if (cand / "worlds").exists() or (cand / "tasks").exists():
            ...
            break
    if src_data is None:
        # Fall back: if src_root/data exists, prefer it.
        src_data = src_root / "data" if (src_root / "data").exists() else src_root

    dst_data = dst_out_dir / "data"

    # If a link already exists but is wrong / incomplete, replace it.
    if dst_data.exists() or dst_data.is_symlink():
        try:
            ok = (dst_data / "worlds" / "documents.jsonl").exists() or (dst_data / "worlds").exists()
        except Exception:
            ok = False
        if ok:
            return
        # Remove stale/incorrect destination.
...
def _rule_map_v1(pivot_type: str) -> tuple[int, int]:
    ...
    - retention_flip: pivot-only heavy helps when pivots are rare
    - entity_switch: hops-only (dependency closure) helps without adding noise
    - constraint_add: keep light to avoid interference from old evidence
    """
...
def main() -> None:
    ...
    ap.add_argument("--pivot_rate", type=float, default=0.2, help="pivot rate for non-retention_flip types")
    ap.add_argument("--pivot_rate_retention_flip", type=float, default=0.5, help="pivot rate for retention_flip")
    ...
    bundle_name = f"goc_policyops_phase12_e2e_commit_anchor_{ts}_{bundle_id}"
    bundle_root = repo_root / "experiment_bundles" / bundle_name
    phase12_root = bundle_root / "phase12"
    data_root = phase12_root / "data"
    runs_root = phase12_root / "runs"
    analysis_root = phase12_root / "analysis"
    ...
    # Network robustness defaults for long parallel OpenAI runs.
    # (Users can override these via env vars without editing code.)
    ...
    for pivot_type in pivot_types:
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            commit_anchor_require_opened=bool(args.commit_anchor_require_opened),
            commit_anchor_fallback_opened=bool(args.commit_anchor_fallback_opened),
            report_json=str(rep_base.relative_to(phase12_root)),
            compare_root=str(out_base.relative_to(phase12_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            fixed_k=4,
            fixed_h=2,
            report_json=str(rep_light.relative_to(phase12_root)),
            compare_root=str(out_light.relative_to(phase12_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            fixed_k=16,
            fixed_h=3,
            report_json=str(rep_heavy.relative_to(phase12_root)),
            compare_root=str(out_heavy.relative_to(phase12_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            pivot_k=16,
            pivot_h=3,
            report_json=str(rep_adapt.relative_to(phase12_root)),
            compare_root=str(out_adapt.relative_to(phase12_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            pivot_k=pk,
            pivot_h=ph,
            report_json=str(rep_rule.relative_to(phase12_root)),
            compare_root=str(out_rule.relative_to(phase12_root)),
        )))

    # Save manifest
    (phase12_root / "run_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    # Analyze
    analyze_cmd = [
        sys.executable,
        str(repo_root / "scripts" / "analyze_phase12_e2e_commit_anchor.py"),
        "--phase12_root", str(phase12_root),
    ...
    # INDEX
    idx_lines: List[str] = []
    idx_lines.append(f"# Phase 12 E2E (commit anchor) bundle: {bundle_name}\n")
    idx_lines.append(
        "- " +
    ...
    idx_lines.append("\n## Outputs")
    idx_lines.append(f"- phase12_root: {phase12_root}")
    idx_lines.append(f"- manifest: {phase12_root / 'run_manifest.json'}")
