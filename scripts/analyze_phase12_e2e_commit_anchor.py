#!/usr/bin/env python3
"""Phase 12 analyzer: end-to-end (episode 1~3) accuracy + cost under commit-anchor-by-evidence.

Commit anchors (episodes 1/2) can be derived from model evidence IDs; strict_e2e_accuracy becomes a stronger measure.

Input: phase12/run_manifest.json + report JSONs produced by policyops.run compare.
Output: phase12_summary.csv/.md and optional figures.
...
def main() -> None:
    ...
    phase12_root = Path(args.phase12_root)
    ...
    manifest = _load_json(phase12_root / "run_manifest.json")
    runs = list(manifest.get("runs") or [])

    rows: List[Dict[str, Any]] = []

    for run in runs:
        report_path = phase12_root / Path(run["report_json"])
    ...
    # Save CSV
    csv_path = out_dir / "phase12_summary.csv"
    df.to_csv(csv_path, index=False)

    # Markdown summary
    md_lines: List[str] = []
    md_lines.append("# Phase 12 E2E summary\n")
