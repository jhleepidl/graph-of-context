...
def _report(rows: Sequence[Dict[str, Any]], predicted_actions: Sequence[str]) -> Dict[str, Any]:
    gold = [str(r.get('best_action') or 'none') for r in rows]
    oracle = float(sum(float(r.get('best_utility') or 0.0) for r in rows) / max(1, len(rows)))
    mean_util = _mean_utility(rows, predicted_actions)
    return {
        'n': int(len(rows)),
        'mean_utility': float(mean_util),
        'oracle_mean_utility': float(oracle),
        'mean_regret': float(oracle - mean_util),
...
def main() -> None:
    ap = argparse.ArgumentParser(description='Train a practical support-closure controller from collected controller-state data.')
    ap.add_argument('--dataset', type=Path, required=True)
    ap.add_argument('--out_model', type=Path, required=True)
    ap.add_argument('--out_report', type=Path, required=True)
    ap.add_argument('--model_type', choices=['logreg', 'tree'], default='tree')
    ap.add_argument('--tree_max_depth', type=int, default=5)
    ap.add_argument('--min_train_rows', type=int, default=20)
    args = ap.parse_args()

    rows = _iter_rows(args.dataset)
    ...
    if not test_rows:
        raise RuntimeError('Dataset has no test rows.')
    ...
    pred_dev = [ACTIONS[int(i)] for i in model.predict(X_dev_arr)]
    pred_test = [ACTIONS[int(i)] for i in model.predict(X_test_arr)]

    report = {
        'model_type': str(args.model_type),
        'feature_names': list(feature_names),
        'dev': _report(dev_rows, pred_dev),
        'test': _report(test_rows, pred_test),
    ...
    print(f"Test mean utility: {report['test']['mean_utility']:.4f}")
    print(f"Test oracle utility: {report['test']['oracle_mean_utility']:.4f}")
    print(f"Test mean regret: {report['test']['mean_regret']:.4f}")
