#!/usr/bin/env python3
"""Phase 14 runner: PolicyOps end-to-end (episode 1~3) evaluation.

Focus:
- pivot_type=entity_switch by default
- Phase 13 stack preserved (pivot gold + avoids + frontier)
- Phase 14 GoC controls enabled by default for E3 pivots:
  - applicability seeding
  - budgeted dependency closure

Outputs a self-contained experiment bundle under experiment_bundles/ and zips it.

Usage:
  python scripts/run_phase14_e2e_entity_switch_bundle.py --dotenv .env

Notes:
- This runner intentionally keeps baseline comparisons unchanged.
- Use --include_ablations to additionally run GoC w/ frontier off and/or closure universe=world.
"""
...
def main() -> None:
    ...
    # Phase 14 knobs
    ap.add_argument("--goc_applicability_seed_enable", action="store_true", default=True)
    ap.add_argument("--no_goc_applicability_seed_enable", action="store_false", dest="goc_applicability_seed_enable")
    ap.add_argument("--goc_applicability_seed_topk", type=int, default=8)
    ap.add_argument("--goc_dependency_closure_enable", action="store_true", default=True)
    ap.add_argument("--no_goc_dependency_closure_enable", action="store_false", dest="goc_dependency_closure_enable")
    ap.add_argument("--goc_dependency_closure_topk", type=int, default=12)
    ap.add_argument("--goc_dependency_closure_hops", type=int, default=1)
    ap.add_argument(
    ...
    bundle_name = f"goc_policyops_phase14_e2e_entity_switch_{ts}_{bundle_id}"
    bundle_root = repo_root / "experiment_bundles" / bundle_name
    phase14_root = bundle_root / "phase14"
    data_root = phase14_root / "data"
    runs_root = phase14_root / "runs"
    analysis_root = phase14_root / "analysis"
    ...
    manifest: Dict[str, Any] = {
        ...
        "goc_applicability_seed_enable": bool(args.goc_applicability_seed_enable),
        "goc_applicability_seed_topk": int(args.goc_applicability_seed_topk),
        "goc_dependency_closure_enable": bool(args.goc_dependency_closure_enable),
        "goc_dependency_closure_topk": int(args.goc_dependency_closure_topk),
        "goc_dependency_closure_hops": int(args.goc_dependency_closure_hops),
        "goc_dependency_closure_universe": str(args.goc_dependency_closure_universe),
    ...
    if args.goc_applicability_seed_enable:
        base_goc_flags += ["--goc_applicability_seed_enable"]
    base_goc_flags += ["--goc_applicability_seed_topk", str(args.goc_applicability_seed_topk)]
    if args.goc_dependency_closure_enable:
        base_goc_flags += ["--goc_dependency_closure_enable"]
    base_goc_flags += [
        "--goc_dependency_closure_topk", str(args.goc_dependency_closure_topk),
        "--goc_dependency_closure_hops", str(args.goc_dependency_closure_hops),
        "--goc_dependency_closure_universe", str(args.goc_dependency_closure_universe),
    ]
    ...
    for pivot_type in pivot_types:
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            goc_enable_avoids=bool(args.goc_enable_avoids),
            goc_avoids_mode=str(args.goc_avoids_mode),
            report_json=str(rep_base.relative_to(phase14_root)),
            compare_root=str(out_base.relative_to(phase14_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            fixed_k=4,
            fixed_h=2,
            report_json=str(rep_light.relative_to(phase14_root)),
            compare_root=str(out_light.relative_to(phase14_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            fixed_k=16,
            fixed_h=3,
            report_json=str(rep_heavy.relative_to(phase14_root)),
            compare_root=str(out_heavy.relative_to(phase14_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            pivot_k=16,
            pivot_h=3,
            report_json=str(rep_adapt.relative_to(phase14_root)),
            compare_root=str(out_adapt.relative_to(phase14_root)),
        )))
        ...
        manifest["runs"].append(asdict(RunEntry(
            ...
            pivot_k=pk,
            pivot_h=ph,
            report_json=str(rep_rule.relative_to(phase14_root)),
            compare_root=str(out_rule.relative_to(phase14_root)),
        )))
        ...
        if args.include_ablations:
            ...
            manifest["runs"].append(asdict(RunEntry(
                ...
                pivot_k=pk,
                pivot_h=ph,
                report_json=str(rep_rule_nof.relative_to(phase14_root)),
                compare_root=str(out_rule_nof.relative_to(phase14_root)),
            )))
            ...
            manifest["runs"].append(asdict(RunEntry(
                ...
                pivot_k=pk,
                pivot_h=ph,
                report_json=str(rep_rule_world.relative_to(phase14_root)),
                compare_root=str(out_rule_world.relative_to(phase14_root)),
            )))

    # Save manifest
    (phase14_root / "run_manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    # Analyze
    analyze_cmd = [
        sys.executable,
        str(repo_root / "scripts" / "analyze_phase13_e2e_universe_frontier.py"),
        "--phase13_root", str(phase14_root),
    ...
    # INDEX
    idx_lines: List[str] = []
    idx_lines.append(f"# Phase 14 E2E (entity_switch focus) bundle: {bundle_name}\n")
    idx_lines.append(
        "- "
    ...
    idx_lines.append(
        "- "
        + f"goc_applicability_seed_enable={bool(args.goc_applicability_seed_enable)} "
        + f"goc_applicability_seed_topk={int(args.goc_applicability_seed_topk)} "
        + f"goc_dependency_closure_enable={bool(args.goc_dependency_closure_enable)} "
        + f"goc_dependency_closure_topk={int(args.goc_dependency_closure_topk)} "
        + f"goc_dependency_closure_hops={int(args.goc_dependency_closure_hops)} "
        + f"goc_dependency_closure_universe={args.goc_dependency_closure_universe}"
    )
    ...
    idx_lines.append("\n## Outputs")
    idx_lines.append(f"- phase14_root: {phase14_root}")
    idx_lines.append(f"- manifest: {phase14_root / 'run_manifest.json'}")
