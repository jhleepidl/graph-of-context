...
from policyops.analysis import analyze_slot_breakdown
from policyops.generator import generate_world_and_tasks
from policyops.run import cmd_compare


def test_slot_breakdown_report_created(tmp_path: Path) -> None:
    ...
    csv_path, md_path = analyze_slot_breakdown(report_path, method="goc")
    assert csv_path.exists()
    assert md_path.exists()
    md_text = md_path.read_text(encoding="utf-8")
    assert "slot_term" in md_text
